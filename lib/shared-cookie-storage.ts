 ...
 * on planmycashflows.com is invisible to app.planmycashflows.com.
 *
 * This adapter writes to a cookie scoped to `.planmycashflows.com` (note the
 * leading dot). The browser automatically includes the cookie on requests
 * to ANY subdomain of planmycashflows.com, so both the marketing site and the
 * CashFlow Planner app see the same session.
 *
 * On localhost (and any host that isn't *.planmycashflows.com), we fall back to
 ...
 * The CashFlow Planner app repo (wealthwise2) uses the identical scheme, so the
 * two sites can read each other's cookies — keep them in sync.
 */

const COOKIE_CHUNK_SIZE = 3180;

/** Returns "planmycashflows.com" for any *.planmycashflows.com host, else undefined. */
function getSharedCookieDomain(): string | undefined {
  if (typeof window === "undefined") return undefined;
  const host = window.location.hostname;
  if (!host || host === "localhost" || host === "127.0.0.1") return undefined;
  if (host === "planmycashflows.com" || host.endsWith(".planmycashflows.com")) {
    return "planmycashflows.com"; // browser treats this as ".planmycashflows.com" — shared
  }
  return undefined;
}
