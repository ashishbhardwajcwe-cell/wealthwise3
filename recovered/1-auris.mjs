...
function rowToRecord(cells) {
  if (cells.length < 12) return null;                 // header/rsubtotal/short rows -> skip
  ...
  return { provider: provider.trim(), ia: ia.trim(),
    aumCr: num(aum), m1: num(m1), m3: num(m3), m6: num(m6),
...
async function main() {
  ...
  const header = ["strategyName","manager","category","aumCr","minInvestmentL",
    "returns1m","returns3m","returns6m","returns1y","returns2y","returns3y","returns4y","returns5y",
    ...
    for (const svc of SERVICE_TYPES) {
      ...
      for (const r of recs) {
        ...
        const row = [ r.ia, r.provider, mapCategory(st + " " + r.ia), r.aumCr, "50",
          r.m1, r.m3, r.m6, r.y1, r.y2, r.y3, r.y4, r.y5, r.si, "", "", "",
          asOnDate, SOURCE, "" ];
